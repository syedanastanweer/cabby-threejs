...
async function setupViewer() {
  const viewer = new ViewerApp({
    ...
    useRgbm: false,
      isAntialiased: isIOS ? false : true, // Disable antialiasing on iOS for better performance

  });
  const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream;
viewer.renderer.displayCanvasScaling = isIOS ? 1 : Math.min(window.devicePixelRatio, 1);
  ...
  function setupScrollanimation() {
    ...
    tl.to(modelPosition, {
      ...
      // Apply different rotation for mobile devices
      .to(modelRotation, {
        x: isMobile ? 1.57 : 1.57,
        y: isMobile ? -1.58 : 0,
        z: isMobile ? 0 : 0,
        scrollTrigger: {
          trigger: ".second",
